...
export default defineConfig({plugins:[react(),tailwindcss()],define:{__LUMI_BUILD_VERSION__:JSON.stringify(version),__LUMI_BUILD_TIME__:JSON.stringify(buildTime),__LUMI_GIT_HASH__:JSON.stringify(gitHash)},resolve:{alias:[{find:'@lumi/tailwind',replacement:fileURLToPath(new URL('../../tailwind.css',import.meta.url))},{find:'@',replacement:fileURLToPath(new URL('../../',import.meta.url))}],dedupe:['react','react-dom']},
