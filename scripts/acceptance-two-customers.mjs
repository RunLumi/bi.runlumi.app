...
`,'utf8');
 ...
 await step('F1 both upgrade to N+1 without altering customer files',async()=>{
  ...
  for(const pkg of ['core','cloudflare','ui']){
   ...
   j.version='0.1.4';if(pkg==='cloudflare'||pkg==='ui')j.dependencies['@runlumi/core']='0.1.4';
   await writeFile(file,JSON.stringify(j,null,2));
  }
  await writeFile(path.join(bump,'packages/core/src/version.ts'),"export const LUMI_CORE_VERSION = '0.1.4';\nexport const LUMI_EXTENSION_API = 1;\n");
  ...
  for(const dir of [alpha,beta]){
   ...
   assert(/0\.1\.3 -> 0\.1\.4/.test(out.stdout),`upgrade plan must report N -> N+1 for ${dir}`);
   const lock=JSON.parse(await customerFile(dir,'lumi.lock.json'));
   assert.equal(lock.core.version,'0.1.4',`${dir} lock must record N+1`);
  ...
  for(const dir of [alpha,beta]){
   ...
   run(npm,['run','build'],dir);
   run('git',['add','-A'],dir);
   run('git',['-c','user.email=acceptance@lumi.invalid','-c','user.name=Lumi Acceptance','commit','-q','--no-verify','-m','core upgrade 0.1.3 -> 0.1.4'],dir);
  ...
  assert(upgraded.includes('0.1.4'),'vendored manifest must record the new release');
  for(const dir of [alpha,beta]){
 ...
 await step('H1 beta rolls back to REAL N artifacts; alpha stays at N+1',async()=>{
  ...
  assert.equal(betaLock.core.version,'0.1.3','beta records the rollback');
  assert.equal(alphaLock.core.version,'0.1.4','alpha is unchanged by beta rollback');
  const installedCore=JSON.parse(await customerFile(beta,'node_modules/@runlumi/core/package.json'));
  assert.equal(installedCore.version,'0.1.3','beta installed core is the previous release');
