...
export async function generateCustomer({customerId,displayName,envs=['production'],dest,artifactsDir=path.join(root,'artifacts/core'),starterDir=path.join(root,'starter/customer')}){
 ...
 const templatePackage=JSON.parse(await readFile(path.join(starterDir,'package.json.template'),'utf8'));
 if(rootPkg.version!==templatePackage.version)throw new Error(`Starter lock root version ${rootPkg.version} does not match template package ${templatePackage.version}`);
