...
export async function controlRequest(env:Env,original:Request,path:string,body?:unknown,demo=false):Promise<Response>{
  if(!env.CONTROL||typeof env.CONTROL.fetch!=='function')throw new AppError(503,'CONTROL_UNAVAILABLE');
  ...
  try{response=await env.CONTROL.fetch(new Request(`https://control.internal${path}`,{method:body!==undefined?(path==='/control/authorize'?'POST':original.method==='PUT'?'PUT':'POST'):'GET',headers,...(body!==undefined?{body:JSON.stringify(body)}:{}),signal:AbortSignal.timeout(5000)}));}
