...
test('owner completes source authorization, file import, normalization, reviewed publication, export and investigation in the UI',async({page})=>{
 ...
 await page.getByLabel('Câu hỏi phân tích',{exact:true}).fill('Doanh số hàng hóa là bao nhiêu?');await page.getByRole('button',{name:'Hỏi',exact:true}).click();await expect(page.getByText('ANSWERED',{exact:true})).toBeVisible();
