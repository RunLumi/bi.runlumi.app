...
uses Elastic-2.0 under the [root license](../LICENSE); see [scope](../LICENSING.md). Build tools are not browser runtimes. Local
...
dependency versions are unchanged. The build also includes the full product
license in `LICENSE.txt` and attribution in `NOTICE.txt`.
