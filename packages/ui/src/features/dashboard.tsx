...
import {api,dashboardData,message,installationApiBase,type SavedDashboard,type InstallationUser,type Metric} from '../lib/api.ts';
...
export function DashboardPage({user,identity,demo}:{user:InstallationUser;identity:string;demo:boolean}){
 const base=installationApiBase(user);const list=useQuery({queryKey:['dashboards',identity,user.id],queryFn:({signal})=>api<{dashboards:SavedDashboard[]}>(base+'/dashboards',identity,{signal})});
 ...
 <DashboardCanvas key={`${user.id}:${dashboard.id}:${dashboard.revision}:${dashboard.releaseId??''}`} user={user} identity={identity} dashboard={dashboard} from={from} to={to}/>
 ...
 <Editor key={`${user.id}:${dashboard.id}:${dashboard.revision}`} user={user} identity={identity} dashboard={dashboard}/></>;
}
function DashboardCanvas({user,identity,dashboard,from,to}:{user:InstallationUser;identity:string;dashboard:SavedDashboard;from:string;to:string}){
 const client=useQueryClient();const base=installationApiBase(user);
 const query=useQuery({queryKey:['results',identity,user.id,dashboard.id,dashboard.revision,dashboard.configurationRelease,dashboard.configurationRevision,from,to],queryFn:({signal})=>dashboardData(base,identity,dashboard,from,to,signal)});
 const catalog=useQuery({queryKey:['metrics',identity,user.id,dashboard.configurationRelease,dashboard.configurationRevision],queryFn:({signal})=>api<{metrics:Metric[]}>(base+'/metrics',identity,{signal})});
 if(query.isPending||catalog.isPending)return <Loading/>;if(query.isError||catalog.isError)return <ErrorState error={query.error??catalog.error} retry={()=>{client.invalidateQueries({queryKey:['dashboards',identity,user.id]});query.refetch();catalog.refetch();}}/>;
...
function Editor({user,identity,dashboard}:{user:InstallationUser;identity:string;dashboard:SavedDashboard}){
 const client=useQueryClient();const base=installationApiBase(user);const [value,setValue]=useState(JSON.stringify(dashboard.definition,null,2));const [note,setNote]=useState('');
 const mutation=useMutation({mutationFn:async(copy:boolean)=>{let definition:unknown;try{definition=JSON.parse(value);}catch{throw new Error('INVALID_JSON');}return api(base+`/dashboards${copy?'':`/${dashboard.id}`}`,identity,{method:copy?'POST':'PUT',body:copy?{id:`dashboard-${crypto.randomUUID().slice(0,8)}`,definition}:definition,...(!copy?{revision:dashboard.revision}:{})});},onSuccess:()=>{setNote('Đã lưu cấu hình được kiểm tra.');client.invalidateQueries({queryKey:['dashboards',identity,user.id]});}});
 if(user.role==='viewer'||!user.features.includes('dashboard.edit'))return null;
