...
import {IconLayoutSidebarLeftCollapse,IconLayoutSidebarLeftExpand,IconLogout,IconMenu2,IconX} from '@tabler/icons-react';
...
function Gate({identity,onIdentity,epoch,onEpoch,config}:{identity:string;onIdentity:(value:string)=>void;epoch:number;onEpoch:()=>void;config:AppConfig}){
 ...
 return <div className="app-shell"><a className="skip-link" href="#main-content">{config.labels.skipToContent}</a><aside className="sidebar" data-collapsed={sidebarCollapsed} data-mobile-open={sidebarMobileOpen}><div className="sidebar-heading"><a className="brand" href="/"><img src={config.brand.logoSrc} width="28" height="28" alt={config.brand.logoAlt}/><strong>{config.brand.name}</strong></a><div className="sidebar-controls"><button className="sidebar-collapse" type="button" aria-label={sidebarCollapsed?'Mở rộng thanh điều hướng':'Thu gọn thanh điều hướng'} aria-expanded={!sidebarCollapsed} aria-controls="primary-navigation" title={sidebarCollapsed?'Mở rộng thanh điều hướng':'Thu gọn thanh điều hướng'} onClick={()=>setSidebarCollapsed(value=>!value)}>{sidebarCollapsed?<IconLayoutSidebarLeftExpand aria-hidden="true"/>:<IconLayoutSidebarLeftCollapse aria-hidden="true"/>}</button><button className="sidebar-mobile-toggle" type="button" aria-label={sidebarMobileOpen?'Đóng thanh điều hướng':'Mở thanh điều hướng'} aria-expanded={sidebarMobileOpen} aria-controls="primary-navigation" title={sidebarMobileOpen?'Đóng thanh điều hướng':'Mở thanh điều hướng'} onClick={()=>setSidebarMobileOpen(value=>!value)}>{sidebarMobileOpen?<IconX aria-hidden="true"/>:<IconMenu2 aria-hidden="true"/>}</button></div></div><p className="workspace-label">{config.brand.workspaceLabel}</p>
 <p className="user-label">{session.installation.name??user.name}</p>
 <div className="sidebar-navigation" data-mobile-open={sidebarMobileOpen}><AppNavigation pages={visible} groups={config.navGroups??[]} label={config.labels.navAriaLabel} onNavigate={()=>setSidebarMobileOpen(false)}/></div>
