...
import {SEMANTIC_VERSION,orderMetrics,settlementMetrics,minor,quantityUnits,formatQuantity,type NormalizedExport,type OrderObservation,type SettlementObservation,type InventoryObservation} from './commerce-model.ts';
...
export function assembleCommerceReport(inputs:ReportInput[],mapping:IdentityMapping|null,controls:SourceControl[]) {
  ...
  for(const input of inputs){
    const d=input.data;if(d.tenantId!==first.tenantId)throw new AppError(403,'CROSS_TENANT_PUBLICATION');
    if(d.window.from!==first.window.from||d.window.toExclusive!==first.window.toExclusive)throw new AppError(422,'PUBLICATION_WINDOW_MISMATCH');
  ...
  return {contract:'lumi.commerce.report.v1' as const,semanticVersion:SEMANTIC_VERSION,tenantId:first.tenantId,currency:'VND' as const,taxBasis:first.taxBasis,timezone:first.timezone,recognitionPolicy:first.recognitionPolicy,window:first.window,sources,checks,exclusions,warnings,qualityState:'PROVISIONAL' as const,sourceCompletenessCertified:false,merchantVerified:false,liveProviderVerified:false,
