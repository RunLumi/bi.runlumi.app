...
export interface SourceScope {provider:string;sourceAccountId:string;resourceType:string;eventType?:string;window:{from:string;toExclusive:string};schemaFingerprint:string}
...
export interface NormalizedExport {
  ...
  provider:Provider;sourceAccountId:string;resourceType:Resource;
...
export async function sourceKey(scope:Pick<SourceScope,'provider'|'sourceAccountId'|'resourceType'>,sourceId:string){
  // Identity excludes a connection/token ID: reinstalling cannot mint a second object.
  // Installation-local by construction: every value comes from this installation's data.
  return 'so_'+(await sha256(JSON.stringify([scope.provider,scope.sourceAccountId,scope.resourceType,text(sourceId,512)]))).slice(0,48);
}
export async function normalizeCommerceExport(raw:unknown,scope:SourceScope):Promise<NormalizedExport> {
  if(scope.eventType!==undefined&&scope.eventType!=='snapshot')throw new AppError(422,'EXPLICIT_SNAPSHOT_REQUIRED');
  ...
  const out:NormalizedExport={contract:COMMERCE_CONTRACT,normalizerVersion:NORMALIZER_VERSION,provider,sourceAccountId,resourceType,currency:'VND',taxBasis:TAX_BASIS,timezone:'Asia/Ho_Chi_Minh',recognitionPolicy:'fulfilled-order-cohort-v1',window,observedAt,orders:[],settlements:[],inventory:[]};
