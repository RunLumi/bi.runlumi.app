...
import {parseCommerceQuery,queryCommerceReport,assertCommerceMetricScope} from './commerce-query.ts';
...
/** Curated, deterministic question path. No model, LLM or arbitrary SQL is
 * involved; the supported scope is recognized-order sales and count only. */
export async function answerCommerceQuestion(db:Database,actor:InstallationUser,input:unknown){
 ...
 const publication=await readCommerceQueryPublication(db,text((input as Record<string,unknown>).dataVersion,128));const query=parseCommerceQuery({contract:'lumi.query.v1',metrics:[{id:metric,version:1}],dimensions:[],filters:[],limit:1,consistency:'published',dataVersion:publication.id});assertCommerceMetricScope(actor.role==='owner'?'owner':actor.role,query.metrics);
 const result=queryCommerceReport(publication.report as unknown as Record<string,unknown>,query);return {status:'ANSWERED',mode:'curated-deterministic',llmInvolved:false,interpretation:metric==='net_merchandise_sales'?'Doanh số hàng hóa thuần':'Số đơn được ghi nhận',claims:[{metricId:metric,value:(result as {metrics?:Record<string,{value:string|null}>}).metrics?.[metric]?.value??null,publicationId:publication.id,contentHash:publication.contentHash}],semanticPlan:{metric,version:1,dataVersion:publication.id},limitations:publication.report.warnings,suggestedNextSteps:['Kiểm tra nguồn và phạm vi trước khi lưu hoặc tạo quyết định.']};
}
