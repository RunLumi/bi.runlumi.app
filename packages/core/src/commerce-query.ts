...
export function queryCommerceReport(report:Record<string,unknown>,query:CommerceQuery):unknown{
 if(query.comparison){const {comparison:_,...base}=query;const current=base;const prior={...base,filters:[{field:'business_date' as const,op:'range' as const,value:[query.comparison.from,query.comparison.toExclusive] as [string,string]}]};return {current:queryCommerceReport(report,current),comparison:queryCommerceReport(report,prior)};}
 if(!query.dimensions.length&&query.filters.length&&(report.orders as OrderObservation[]|undefined)){const range=query.filters[0]!.value;const rows=(report.orders as OrderObservation[]).filter(order=>order.recognizedAt!==null&&order.recognizedAt>=range[0]&&order.recognizedAt<range[1]);const values=orderMetrics(rows),result:Record<string,{value:string|null;unit:CommerceMetricUnit;metricVersion:1}>={};for(const requested of query.metrics){const definition=metricMap.get(requested.id)!;const value=requested.id==='net_merchandise_sales'?values.net_merchandise_sales:requested.id==='recognized_order_count'?values.recognized_order_count:null;result[requested.id]={value,unit:definition.unit,metricVersion:1};}return result;}
